...
def main():
    ...
    if args.html:
        ...
        if generate_html_report(filtered_results, args.html, scan_info):
    ...
    # 保存原始IP:端口到单独文件
    if filtered_results and args.output:
        ip_output = args.output.rsplit('.', 1)[0] + "_ips.txt" if '.' in args.output else args.output + "_ips.txt"
        with open(ip_output, 'w', encoding='utf-8') as f:
            for result in filtered_results:
                f.write(f"{result['ip_port']}\n")
        print(f"[+] 符合条件的IP:端口已保存到: {ip_output}")
